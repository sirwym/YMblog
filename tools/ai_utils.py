...
5. **val.py 性能生死线 (Validator Performance)**：
   - **连通性校验**：
     - ✅ **首选 DSU (并查集)**：实现简单，天然 $O(M \alpha(N))$，最稳健。
     - ⚠️ **若用 BFS/DFS**：**必须**先构建 **邻接表 (Adjacency List)**。
       - ❌ **严禁**在 BFS 内部用 `for v in range(1, n+1)` 寻找邻居，这是 $O(N^2)$ 必炸！
   - **树结构校验**：
     - 若输入格式为“父节点数组”且要求 $p_i < i$，直接检查数值范围即可判定连通，**无需**跑图算法。
"""
}
